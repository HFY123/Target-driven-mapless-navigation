...
class PPO():
    ...
    def __init__(self, input_shape, action_space,
                 learning_rate=3e-3, lr_decay=0.998, epsilon=0.2,
        ...
        # Policy loss
        adv = tf.expand_dims(self.advantage, axis=-1)  #Returns a tensor with a length 1 axis inserted at index axis  --hfy
    ...
    def predict(self, input_states, greedy=False, write_to_summary=False):
        ...
        if len(input_states.shape) != 2:
            input_states = [input_states]  #69  --hfy
        # Predict action
